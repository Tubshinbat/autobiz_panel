...
const Add = (props) => {
  ...
  useEffect(() => {
    if (props.success) {
      ...
      props.clear();
      setTimeout(() => props.history.replace("/freemod"), 2000);
    }
  }, [props.success]);

  useEffect(() => {
    setForm(() => ({ model: props.freemod && props.freemod.model }));
  }, [props.freemod]);
  ...
  const init = () => {
    props.clear();
    props.getFreemod(props.match.params.id);
  };
  ...
  const addClick = () => {
    ...
    allCheck() === true
      ? props.updateFreemods(props.match.params.id, sendData)
...
const mapStateToProps = (state) => {
  return {
    ...
    freemod: state.freemodReducer.freemod,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    updateFreemods: (id, data) => dispatch(actions.updateFreemods(id, data)),
    getFreemod: (id) => dispatch(actions.getFreemods(id)),
