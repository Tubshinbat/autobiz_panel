...
// ACTIONS
import * as actions from "../../redux/actions/freemodActions";
...
const Freemod = (props) => {
  ...
  const init = () => {
    ...
    setChkBox(() => []);
    props.clear();
    props.loadFreemods(`select=${select}`);
  };

  const addClick = () => {
    props.history.push("/freemod/add");
  };
  ...
  return (
    <Section>
      <MetaTags>
        <title> Freemod аралын дугаар | WEBR Control Panel</title>
        <meta
          name="description"
          content="Freemod аралын дугаар | WeBR control panel"
        />
        <meta
          property="og:title"
          content="Freemod аралын дугаар | web control panel"
        />
      </MetaTags>
      <PageTitle name="Freemod аралын дугаар" />
      <div className="row">
        <div className={css.PanelControl}>
          <div className="col-md-4">
            <div className={css.PanelTabelHeader}>
              ...
              >
                <i className="fas fa-plus-circle"></i> Freemod аралын дугаар
              </button>
              <button
                name="refresh"
                onClick={() => init()}
...
const mapStateToProps = (state) => {
  return {
    freemods: state.freemodReducer.freemods,
    pageLast: state.freemodReducer.paginationLast,
    loading: state.freemodReducer.loading,
    success: state.freemodReducer.success,
    error: state.freemodReducer.error,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    loadFreemods: (query) => dispatch(actions.loadFreemods(query)),
    loadPagination: (pageLast) => dispatch(actions.loadPagination(pageLast)),
    deleteMultFreemods: (ids) => dispatch(actions.deleteMultFreemods(ids)),
  };
};
