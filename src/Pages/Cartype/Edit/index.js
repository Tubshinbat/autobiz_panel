...
const Add = (props) => {
  ...
  useEffect(() => {
    if (props.carType) {
      setForm(() => ({ ...props.carType }));
      if (props.carType.image) {
        setImage(props.carType.image);
      }
    }
  }, [props.carType]);
  ...
  const init = () => {
    ...
    props.removePhotos();
    setForm(() => ({}));
    props.getCartype(props.match.params.id);
  };
  ...
  const addClick = () => {
    ...
    allCheck() === true
      ? props.updateCartype(props.match.params.id, sendData)
...
const mapDispatchToProps = (dispatch) => {
  return {
    getCartype: (id) => dispatch(actions.getCartype(id)),
    updateCartype: (id, data) => dispatch(actions.updateCartype(id, data)),
