...
const News = (props) => {
  ...
  const [select, setSelect] = useState("status name image createAt");
  ...
  const init = () => {
    ...
    setChkBox(() => []);
    props.clear();
    props.loadCartypes(`select=${select}`);
  };

  const addClick = () => {
    props.history.push("/car_type/add");
  };
  ...
  return (
    <Section>
      <MetaTags>
        <title> Машины төрөл | WEBR Control Panel</title>
        <meta name="description" content="Машины төрөл | WeBR control panel" />
        <meta property="og:title" content="Машины төрөл | web control panel" />
      </MetaTags>
      <PageTitle name="Машины төрөл" />
      <div className="row">
        <div className={css.PanelControl}>
          <div className="col-md-4">
            <div className={css.PanelTabelHeader}>
              ...
              >
                <i className="fas fa-plus-circle"></i> Машины төрөл
              </button>
              <button
                name="refresh"
                onClick={() => init()}
...
const mapStateToProps = (state) => {
  return {
    ...
    pageLast: state.carTypeReducer.paginationLast,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    loadCartypes: (query) => dispatch(actions.loadCartypes(query)),
    loadPagination: (pageLast) => dispatch(actions.loadPagination(pageLast)),
    deleteMultCartype: (ids) => dispatch(actions.deleteMultCartype(ids)),
  };
};
