...
const Edit = (props) => {
  ...
  const convertFromdata = () => {
    ...
    Object.keys(formData).map((index) => {
      if (index === "pictures" || index === "oldPicture") {
        if (formData[index])
  ...
  const textAreaChange = (event) => {
    setForm((bf) => ({ ...bf, description: event }));
    checkFrom("description", event);
  };
